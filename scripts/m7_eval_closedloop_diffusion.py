...
class LangBCPolicy(nn.Module):
    """M7 DiffusionPolicy for inference.

    Same module name kept so the existing eval helpers (env make, summary write, etc.) can reuse the structure.
    forward(noisy_actions, timesteps, obs, lang_emb, image_emb) -> (eps_pred, task_logits).
    """
    ...
    def _global_cond(self, obs, lang_emb, image_emb, timesteps):
        ...
        return torch.cat([t_emb, cond], dim=-1)

    def forward(self, noisy_actions, timesteps, obs, lang_emb, image_emb):
        global_cond = self._global_cond(obs, lang_emb, image_emb, timesteps)
...
class VisionEncoder:
    """Lazy-loaded CLIP image encoder kept around for the full eval run."""
...
def sample_action_chunk(
    ...
    """One DDIM sampling pass -> denormalized action chunk [T_chunk, A]."""
    ...
    chunk_raw = chunk_norm * stats["action_std"] + stats["action_mean"]
...
def run_episode(
    ...
    for t in range(max_steps):
        ...
        if policy_name == "random":
            ...
            if scheduler is None:
                raise RuntimeError("phase_bc (diffusion) requires a DDIM scheduler.")
            ...
            if lang_emb is None:
                raise RuntimeError("phase_bc with VLA model requires a precomputed lang_emb.")
            if vision_encoder is None:
                raise RuntimeError("phase_bc with VLA model requires a vision_encoder.")
        ...
        cube_xy_goal_dists.append(
            float(np.linalg.norm(features["cube_pos"][:2] - features["goal_pos"][:2]))
        )

        if (term_value or trunc_value) and not ignore_termination:
            break

    xy_arr = np.asarray(cube_xy_goal_dists, dtype=np.float32) if cube_xy_goal_dists else np.zeros((0,), dtype=np.float32)

    def sustained_in(threshold: float, n: int) -> bool:
        if xy_arr.size < n:
            return False
        mask = (xy_arr <= threshold).astype(np.int32)
        run = 0
        for m in mask:
            if m:
                run += 1
    ...
    return {
        ...
        "final_cube_xy_goal_dist": float(xy_arr[-1]) if xy_arr.size else None,
        "min_cube_xy_goal_dist": float(xy_arr.min()) if xy_arr.size else None,
        "xy_in_100mm_steps": int((xy_arr <= 0.10).sum()) if xy_arr.size else 0,
        "xy_in_50mm_steps": int((xy_arr <= 0.05).sum()) if xy_arr.size else 0,
        "xy_in_25mm_steps": int((xy_arr <= 0.025).sum()) if xy_arr.size else 0,
        "xy_sustained_100mm_10": sustained_in(0.10, 10),
        "xy_sustained_50mm_10": sustained_in(0.05, 10),
        "xy_sustained_25mm_10": sustained_in(0.025, 10),
        ...
        "num_replans": len(replan_steps_log),
    }
...
def evaluate_policy(
    ...
    env = gym.make(
        ...
        render_mode="rgb_array" if vision_encoder is not None else None,
        human_render_camera_configs=dict(width=render_resolution, height=render_resolution),
    ...
    return {
        ...
        "mean_final_cube_xy_goal_dist": aggregate_float_mean(episodes, "final_cube_xy_goal_dist"),
        "mean_min_cube_xy_goal_dist": aggregate_float_mean(episodes, "min_cube_xy_goal_dist"),
        "xy_sustained_100mm_10_rate": aggregate_bool_rate(episodes, "xy_sustained_100mm_10"),
        "xy_sustained_50mm_10_rate": aggregate_bool_rate(episodes, "xy_sustained_50mm_10"),
        "xy_sustained_25mm_10_rate": aggregate_bool_rate(episodes, "xy_sustained_25mm_10"),
        "mean_xy_in_100mm_steps": aggregate_int_mean(episodes, "xy_in_100mm_steps"),
        "mean_xy_in_50mm_steps": aggregate_int_mean(episodes, "xy_in_50mm_steps"),
        "mean_xy_in_25mm_steps": aggregate_int_mean(episodes, "xy_in_25mm_steps"),
        "episodes": episodes,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--ignore-termination",
        action="store_true",
        help="Do not stop the rollout when terminated/truncated; let the policy continue past ManiSkill native success.",
    )

    parser.add_argument("--num-inference-steps", type=int, default=16,
                        help="DDIM denoising steps at inference (lower = faster).")
    parser.add_argument("--action-exec", type=int, default=4,
                        help="How many actions of each sampled chunk to execute before replanning.")
    ...
    parser.add_argument(
        "--skip-random",
        action="store_true",
        help="Skip the random baseline evaluation (saves time on VLA evals).",
    )
    ...
    print(f"[m6-eval] encoding instruction with {args.text_encoder}: {args.instruction!r}")
    ...
    print(f"[m7-eval] loading vision encoder {args.vision_encoder}")
    vision_encoder = VisionEncoder(name=args.vision_encoder, device=device)

    print(f"[m7-eval] DDIM scheduler: train_steps={model.num_train_timesteps} infer_steps={args.num_inference_steps}")
    scheduler = DDIMScheduler(
    ...
    if args.skip_random:
        random_summary = {"policy": "random", "skipped": True, "num_episodes": 0}
    else:
    ...
    summary = {
        "milestone": "M7C",
        "description": (
            "Multi-task Diffusion Policy (VLA) closed-loop evaluation: DDIM sampling + receding horizon."
        ),
        "num_inference_steps": args.num_inference_steps,
        "action_exec": args.action_exec,
    ...
    if not random_summary.get("skipped", False):
        summary["comparison"] = {
