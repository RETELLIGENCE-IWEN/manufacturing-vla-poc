"""M8a: Multi-task BC with PCGrad (Yu et al. 2020) on the multi-task VLA dataset.

PCGrad core idea:
    For each pair of task gradients (g_i, g_j), if cos(g_i, g_j) < 0 the tasks
    are pulling the shared parameters in conflicting directions. PCGrad projects
    g_i onto the normal plane of g_j to remove the conflicting component, then
    sums (or averages) the projected gradients before the optimizer step.

This script is forked from scripts/m5_1_train_bc_lang_aux.py:
- the BC loss is decomposed into per-task losses
- per-task gradients are flattened, pairwise-projected (random order, mean),
  and unflattened back into params.grad
- the auxiliary task_id CE loss is added on top (single backward, shared across tasks)

The model architecture is unchanged (M5.1 LangBCPolicyAux). Only the optimizer
update rule changes. So this isolates the optimization-side fix from any
...
class VLAAuxDataset(Dataset):
    ...
    def _build_phase_weights(progress, early_end, mid_end, early_weight, mid_weight, late_weight, normalize):
        if not (0.0 < early_end < mid_end < 1.0):
            raise ValueError(f"0 < early < mid < 1 required, got {early_end}, {mid_end}")
...
# ---------- model (same as M6.2 VLAPolicyAux) ----------
...
class VLAPolicyAux(nn.Module):
    ...
    def forward(self, obs, lang_emb, image_emb):
        ...
        return self.net(torch.cat([obs, lang_p, image_p], dim=-1))

    def forward_with_aux(self, obs, lang_emb, image_emb):
        lang_p = F.relu(self.lang_proj(lang_emb))
        image_p = F.relu(self.image_proj(image_emb))
        action = self.net(torch.cat([obs, lang_p, image_p], dim=-1))
        return action, self.task_head(lang_p)
...
def _flatten_grads(params):
    flat = []
    for p in params:
        flat.append((p.grad if p.grad is not None else torch.zeros_like(p)).reshape(-1))
    return torch.cat(flat, dim=0)


def _unflatten_to_grads(flat, params):
    idx = 0
    for p in params:
        n = p.numel()
...
@torch.no_grad()
def pcgrad_project(task_grads: list) -> torch.Tensor:
    """Yu et al. 2020 PCGrad. Returns the summed projected gradient.

    For each task gradient g_i, project away any conflicting component with
    every other task's original gradient. Iterating in random order each call
    (as in the paper) helps reduce bias.
    """
    num_tasks = len(task_grads)
    if num_tasks <= 1:
        return torch.stack(task_grads).sum(dim=0)

    original = [g.clone() for g in task_grads]
    ...
    for i in range(num_tasks):
        ...
        for j in idxs:
            if i == j:
                continue
            g_j = original[j]
            dot = torch.dot(pc_grads[i], g_j)
            if dot < 0.0:
                pc_grads[i] = pc_grads[i] - dot / (g_j.dot(g_j) + 1e-12) * g_j

    return torch.stack(pc_grads).sum(dim=0)


def weighted_action_mse(pred, target, sample_weights, channel_w):
    sq = (pred - target) ** 2 * channel_w[None, :]
    return (sq.mean(dim=1) * sample_weights).mean()
...
def train_one_epoch_pcgrad(
    ...
    tot_loss = tot_bc = tot_aux = 0.0
    count = 0
    num_conflicts_seen = 0
    ...
    for obs, lang_emb, image_emb, actions, w, task_int in loader:
        ...
        # ---- per-task BC gradient via PCGrad
        task_grads = []
        task_loss_vals = []
        for t in range(num_tasks):
            ...
            if not mask.any():
                continue
            pred, _ = model.forward_with_aux(obs[mask], lang_emb[mask], image_emb[mask])
            tl = weighted_action_mse(pred, actions[mask], w[mask], channel_w)
            optimizer.zero_grad(set_to_none=True)
            tl.backward(retain_graph=False)
            g = _flatten_grads(params).detach().clone()
            task_grads.append(g)
            task_loss_vals.append(float(tl.item()) * int(mask.sum().item()))
        ...
        if pcgrad_enabled:
            for i in range(len(task_grads)):
                for j in range(i + 1, len(task_grads)):
                    if torch.dot(task_grads[i], task_grads[j]) < 0:
                        num_conflicts_seen += 1
            combined_bc = pcgrad_project(task_grads) / len(task_grads)
        else:
            combined_bc = torch.stack(task_grads).mean(dim=0)

        # ---- aux CE gradient (whole batch)
        optimizer.zero_grad(set_to_none=True)
        _, task_logits = model.forward_with_aux(obs, lang_emb, image_emb)
        aux_loss = F.cross_entropy(task_logits, task_int)
        (aux_weight * aux_loss).backward()
        aux_grad = _flatten_grads(params).detach().clone()

        # ---- combined gradient -> params.grad
        final_grad = combined_bc + aux_grad
        _unflatten_to_grads(final_grad, params)
        ...
        n = obs.shape[0]
        bc_loss_val = sum(task_loss_vals) / max(1, n)
        loss_val = bc_loss_val + aux_weight * float(aux_loss.item())
        tot_loss += loss_val * n
        tot_bc += bc_loss_val * n
        tot_aux += float(aux_loss.item()) * n
        count += n

    return {
        "total_loss": tot_loss / max(1, count),
        "bc_loss": tot_bc / max(1, count),
        "aux_loss": tot_aux / max(1, count),
        "num_conflicts_seen": num_conflicts_seen,
    }
...
def parse_config(cfg):
    t = cfg["training"]; m = cfg["model"]; l = cfg["loss"]; lc = cfg.get("lang", {}); ic = cfg.get("image", {}); ac = cfg.get("aux", {}); pc = cfg.get("pcgrad", {})
    return TrainConfig(
        ...
        pcgrad_enabled=bool(pc.get("enabled", True)),
    )
...
def main():
    ...
    print(f"[m8a] tasks={task_id_strings} pcgrad={cfg.pcgrad_enabled}")

    train_raw = VLAAuxDataset(
    ...
    model = VLAPolicyAux(obs_dim, lang_emb_dim, cfg.lang_proj_dim, image_emb_dim, cfg.image_proj_dim,
                         action_dim, num_tasks, cfg.hidden_dims, cfg.dropout).to(device)
    ...
    with (run_dir / "metadata.json").open("w", encoding="utf-8") as f:
        json.dump({"milestone": "M8A", "config": asdict(cfg), "obs_dim": obs_dim, "action_dim": action_dim,
                   "num_tasks": num_tasks, "task_id_strings": task_id_strings}, f, indent=2)
    ...
    with curve.open("w", encoding="utf-8", newline="") as f_csv:
        writer = csv.DictWriter(f_csv, fieldnames=["epoch","train_total","train_bc","train_aux","val_mse_norm","val_task_acc","conflicts"])
        writer.writeheader()
        for epoch in range(1, cfg.num_epochs + 1):
            tm = train_one_epoch_pcgrad(model, train_loader, optimizer, device, cfg.grad_clip_norm, channel_w, cfg.aux_weight, num_tasks, cfg.pcgrad_enabled)
            ...
            writer.writerow({"epoch": epoch,"train_total": tm["total_loss"],"train_bc": tm["bc_loss"],"train_aux": tm["aux_loss"],
                             "val_mse_norm": vm["mse_norm"],"val_task_acc": vm["task_acc"],"conflicts": tm["num_conflicts_seen"]})
            f_csv.flush()
            if vm["mse_norm"] < best:
                ...
                torch.save({"model_state_dict": model.state_dict(),
                            ...
                            "training_type": "m8a_bc_pcgrad",
                            "pcgrad_enabled": cfg.pcgrad_enabled}, run_dir / "best_model.pt")
            if epoch == 1 or epoch % cfg.print_every_epochs == 0 or epoch == cfg.num_epochs:
                print(f"[ep {epoch:04d}] bc={tm['bc_loss']:.5f} aux={tm['aux_loss']:.4f} conflicts={tm['num_conflicts_seen']} val_mse={vm['mse_norm']:.5f} task_acc={vm['task_acc']:.3f}")
    ...
    final = {"best_epoch": best_epoch, "best_val_mse_norm": best, "best_val_task_acc": best_task_acc,
             ...
             "pcgrad_enabled": cfg.pcgrad_enabled, "num_tasks": num_tasks, "task_id_strings": task_id_strings}
    with (run_dir / "metrics.json").open("w", encoding="utf-8") as f:
        json.dump(final, f, indent=2)

    print("[done] M8A BC+PCGrad training complete")
