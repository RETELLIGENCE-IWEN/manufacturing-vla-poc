...
class LangBCPolicy(nn.Module):
    """M8b PerTaskVLAPolicyAux for inference.

    Shared trunk + per-task action heads. The task_id is inferred from the
    instruction (via the task_head over lang_proj) once at startup and used
    to select the head for every step.
    """
    ...
    def __init__(
        ...
        self.action_heads = nn.ModuleList([nn.Linear(in_dim, action_dim) for _ in range(num_tasks)])
        self.inferred_task_id: int | None = None

    def predict_task_id(self, lang_emb: torch.Tensor) -> int:
        lang_p = F.relu(self.lang_proj(lang_emb))
        logits = self.task_head(lang_p)
        return int(logits.argmax(dim=-1).item())

    def forward(self, obs: torch.Tensor, lang_emb: torch.Tensor, image_emb: torch.Tensor) -> torch.Tensor:
        # interface kept compatible with the m6 eval call site; task_id is inferred lazily once.
        if self.inferred_task_id is None:
            self.inferred_task_id = self.predict_task_id(lang_emb)
        lang_p = F.relu(self.lang_proj(lang_emb))
        image_p = F.relu(self.image_proj(image_emb))
        feats = self.trunk(torch.cat([obs, lang_p, image_p], dim=-1))
        return self.action_heads[self.inferred_task_id](feats)
...
def predict_action(
    ...
    pred_norm = model(obs_t, lang_t, image_t).cpu().numpy()[0].astype(np.float32)
...
def main() -> None:
    ...
    print(f"[m6-eval] loading vision encoder {args.vision_encoder}")
    ...
    summary = {
        "milestone": "M6C",
        "description": (
            "Multi-task VLA closed-loop evaluation: state + lang + image (CLIP visual)."
        ),
