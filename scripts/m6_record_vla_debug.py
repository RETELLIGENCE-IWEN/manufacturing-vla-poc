...
class VLAPolicy(nn.Module):
    """M6 VLA inference-only policy with the auxiliary task_head also loaded."""
...
def main() -> None:
    ...
    parser.add_argument("--ignore-termination", action="store_true",
                        help="Do not stop the rollout when terminated/truncated fires; keep stepping until max_steps. "
                             "Useful to see the policy's behavior past the ManiSkill success criterion (e.g. boundary tap).")
